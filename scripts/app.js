class CryptoLandApp {
    constructor() {
        ...
        this.levelStatuses = new Array(15).fill(0); // 0 - закрыт, 1 - открыт
    ...
    async init() {
        // ===== НОВОЕ: Проверяем, был ли уже подключен кошелек =====
        try {
    ...
    // ===== ФУНКЦИЯ: ОТОБРАЖЕНИЕ АДРЕСА В ШАПКЕ =====
    updateHeaderAddress() {
    ...
    // ===== ИСПРАВЛЕННАЯ ФУНКЦИЯ updateAllText =====
    updateAllText() {
        const t = CONFIG.TRANSLATIONS[this.currentLanguage];
        ...
        // ===== ОБНОВЛЯЕМ БЛОК ОБЩЕГО ОБОРОТА ПРИ СМЕНЕ ЯЗЫКА =====
        ...
        // ===== ВАЖНО: Перерисовываем таблицу уровней при смене языка =====
        this.renderLevels();
    }
    ...
    async renderLevels() {
        ...
        // ===== ВАЖНО: Всегда создаем/обновляем блок общего оборота =====
        const turnoverBlock = document.getElementById('totalStructureTurnover');
        if (turnoverBlock) {
            turnoverBlock.innerHTML = `
                <div class="turnover-block">
        ...
        if (!modal) {
            ...
            modal.innerHTML = `
                ...
                <div class="modal-body">
                    ...
                    <div class="deposits-status-summary">
                        <div class="summary-item">
                            <span class="summary-label">${t.total_active}:</span>
                            <span class="summary-value">${activeDeposits.length}</span>
                        </div>
                        <div class="summary-item">
                            <span class="summary-label">${t.total_amount}:</span>
                            <span class="summary-value">${this.utils.formatNumber(totalAmount)} USDT</span>
                        </div>
                    </div>
            ...
            modal.querySelector('.modal-close').addEventListener('click', () => this.hideModal('depositsStatusModal'));
            document.getElementById('closeStatusModal').addEventListener('click', () => this.hideModal('depositsStatusModal'));
        }
        
        document.getElementById('depositsStatusList').innerHTML = depositsHtml;
