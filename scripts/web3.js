class CryptoLandWeb3 {
    ...
    // ===== НОВАЯ ФУНКЦИЯ: Проверка, есть ли уже подключенный аккаунт =====
    async isWalletConnected() {
        if (!window.ethereum) return false;
        
        try {
            // Пытаемся получить аккаунты без запроса на подключение
    ...
    async initMetaMask() {
        if (window.ethereum) {
            ...
            // Сначала проверяем, есть ли уже подключенный аккаунт
            const accounts = await window.ethereum.request({ method: 'eth_accounts' });
            
            if (accounts.length > 0) {
                // Уже есть подключенный аккаунт
                this.account = accounts[0];
                this.networkId = await this.web3.eth.net.getId();
                await this.checkNetwork();
                await this.initContracts();
                ...
                // Нет подключенного аккаунта - запрашиваем подключение
    ...
    setupMetaMaskEvents() {
        window.ethereum.on('accountsChanged', (accounts) => {
            if (accounts.length === 0) {
                ...
                this.account = accounts[0];
                if (window.app) {
                    // Обновляем данные при смене аккаунта
                    window.app.refreshAllStats();
                    window.app.updateConnectButton(true);
                    window.app.utils.showNotification(
                        window.app.currentLanguage === 'ru' ? 
                    ...
                    if (referrer && window.app.pendingReferrer) {
                        if (this.account.toLowerCase() !== referrer.toLowerCase()) {
                            window.app.utils.showNotification(
                                window.app.currentLanguage === 'ru' ? 
